...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const post = await getArticlePostBySlug(slug);
  ...
  return {
    title: `${post.meta.title} | ATMA Articles`,
    ...
    keywords: post.meta.keywords,
      ...
      url: `https://articles.atma-ai.co.in/${slug}`,
    },
    alternates: {
      canonical: `https://articles.atma-ai.co.in/${slug}`,
    },
  };
}

export default async function ArticlePost({
  params,
}: {
  ...
  const allPosts = await getArticlePosts();
  ...
  return (
    ...
    keywords: post.meta.keywords,
            ...
            mainEntityOfPage: {
              "@type": "WebPage",
              "@id": `https://articles.atma-ai.co.in/${slug}`,
            },
          }),
        }}
      />
      ...
      <main className="flex-grow pt-24 md:pt-32 pb-16 md:pb-24 relative bg-primary-dark">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col lg:flex-row gap-8 lg:gap-12">
          <div className="flex-1 max-w-4xl">
            <Link
              href="/articles"
              className="inline-flex items-center gap-2 text-sm text-muted hover:text-[#F59E0B] transition-colors mb-6 md:mb-8 font-mono"
            >
            ...
            <div className="mb-8 md:mb-12">
              {post.meta.tags && post.meta.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                  {post.meta.tags.map((tag: string) => (
                    <span
                      key={tag}
                      className="px-3 py-1 rounded-full bg-[#F59E0B]/10 text-xs font-mono tracking-wider text-[#F59E0B] border border-[#F59E0B]/20"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <div className="flex flex-wrap gap-3 md:gap-4 text-muted text-sm font-mono">
                ...
                <span className="flex items-center gap-1.5">
                  ...
                  <span className="text-[#F59E0B]">{post.meta.readingTime}</span>
                </span>
              </div>
            </div>
            ...
            {/* ── Author Bio Card ── */}
            {authorInfo && (
              <div className="mt-12 md:mt-16 p-6 md:p-8 rounded-2xl glass-card border border-[#F59E0B]/10">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[#F59E0B] text-sm font-medium mb-2">{authorInfo.role}</p>
            ...
            {relatedPosts.length > 0 && (
              <div className="mt-12 md:mt-16">
                <h2 className="text-2xl font-bold font-heading text-primary-light mb-6">Related Articles</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {relatedPosts.map((related) => (
                    <Link
                      key={related.slug}
                      href={`/articles/${related.slug}`}
                      className="group block p-5 rounded-xl glass-card hover:border-[#F59E0B]/30 transition-all duration-300"
                    >
                      {related.tags && related.tags.length > 0 && (
                        <span className="inline-block px-2 py-0.5 rounded-md bg-[#F59E0B]/10 text-[9px] font-mono tracking-wider text-[#F59E0B] mb-2">
                          {related.tags[0]}
                        </span>
                      )}
                      <h3 className="text-sm font-bold font-heading text-primary-light mb-2 group-hover:text-[#F59E0B] transition-colors line-clamp-2 leading-snug">
                        {related.title}
                      </h3>
                      <div className="flex items-center text-[#F59E0B] text-xs font-medium mt-auto">
                        Read
                ...
                <nav className="flex flex-col gap-2.5">
                  {post.headings.map((heading: { id: string; text: string; level: number }, idx: number) => (
                    <a
                      key={idx}
                      href={`#${heading.id}`}
                      className={`text-sm transition-colors hover:text-[#F59E0B] ${
